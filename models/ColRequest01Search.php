...
class ColRequest01Search extends ColRequest
{
    ...
    public function search($params)
    {
        ...
        $ts = $dt->getTimestamp();
        $query = ColRequest::find()->where(['req_status'=>['$ne'=>'ยุติเรื่อง'],'req_out'=>'F']);
        ...
        if($this->req_date){
            $datas=explode(' - ', $this->req_date);
            if($datas[0]){
                $dates = ['$gte'=>new \MongoDate(strtotime($datas[0].' 00:00:00')),'$lte'=>new \MongoDate(strtotime($datas[1].' 23:59:59'))];
            }

        }else{$dates = $this->req_date;}
        ...
        }else{$daten2 = $this->req_alert2;} 

        if($this->req_alert3){
            $data3=explode(' - ', $this->req_alert3);
            if($data3[0]){
                $daten3 = ['$gte'=>new \MongoDate(strtotime($data3[0].' 00:00:00')),'$lte'=>new \MongoDate(strtotime($data3[1].' 23:59:59'))];
            }

        }else{$daten3 = $this->req_alert3;} 
