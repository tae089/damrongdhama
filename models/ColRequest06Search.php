 ...
 */
class ColRequest06Search extends ColRequest
{
    /**
     * @inheritdoc
     */
    ...
    public function search($params)
    {
        ...
        if($this->req_date){
            ...
            if($data[0]){
                $daten4 = ['$gte'=>new \MongoDate(strtotime($data[0].' 00:00:00')),'$lte'=>new \MongoDate(strtotime($data[1].' 23:59:59'))];
            }

        }else{$daten4 = $this->req_date;} 

        if($this->req_report_date1){
            $data=explode(' - ', $this->req_report_date1);
            if($data[0]){
                $daten4 = ['$gte'=>new \MongoDate(strtotime($data[0].' 00:00:00')),'$lte'=>new \MongoDate(strtotime($data[1].' 23:59:59'))];
            }

        }else{$daten5 = $this->req_report_date1;} 
