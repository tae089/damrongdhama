 ...
 */
class ColRequest04Search extends ColRequest
{
    /**
     * @inheritdoc
     */
    ...
    public function search($params)
    {
        ...
        $ts = $dt->getTimestamp();
        $query = ColRequest::find()->where(['req_out'=>'F']);
        ...
        if($this->req_alert1z){
            $data2=explode(' - ', $this->req_alert1z);
            if($data2[0]){
                $req_alert1z = ['$gte'=>new \MongoDate(strtotime($data2[0].' 00:00:00')),'$lte'=>new \MongoDate(strtotime($data2[1].' 23:59:59'))];
            }

        }else{$req_alert1z = $this->req_alert1z;}

        if($this->req_report_dates){
            $data1=explode(' - ', $this->req_report_dates);
            if($data1[0]){
                $req_report_dates = ['$gte'=>new \MongoDate(strtotime($data1[0].' 00:00:00')),'$lte'=>new \MongoDate(strtotime($data1[1].' 23:59:59'))];
            }

        }else{$req_report_dates = $this->req_report_dates;}
