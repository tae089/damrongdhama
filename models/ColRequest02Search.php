 ...
 */
class ColRequest02Search extends ColRequest
{
    /**
     * @inheritdoc
     */
    ...
    public function search($params)
    {
        ...
        $ts = $dt->getTimestamp();
        $query = ColRequest::find()->where(['req_alert2'=>'','req_report_id'=>'','req_send_date'=>['$ne'=>''],'req_alert1'=>['$ne'=>''],'req_status'=>['$ne'=>'ยุติเรื่อง'],'req_out'=>'F']);
        ...
        if($this->req_alert2){
            $data=explode(' - ', $this->req_alert2);
            if($data[0]){
                $req_alert2 = ['$gte'=>new \MongoDate(strtotime($data[0].' 00:00:00')),'$lte'=>new \MongoDate(strtotime($data[1].' 23:59:59'))];
            }

        }else{$req_alert2 = $this->req_alert2;}

        if($this->req_alert2z){
            $data1=explode(' - ', $this->req_alert2z);
            if($data1[0]){
                $req_alert2z = ['$gte'=>new \MongoDate(strtotime($data1[0].' 00:00:00')),'$lte'=>new \MongoDate(strtotime($data1[1].' 23:59:59'))];
            }

        }else{$req_alert2z = $this->req_alert2z;} 



        $query->andFilterWhere([
            'req_alert2' => $req_alert2,
            'req_alert2z' => $req_alert2z,
            ]);
        ...
        ->andFilterWhere(['like', 'req_alert2_comment', $this->req_alert2_comment]);
